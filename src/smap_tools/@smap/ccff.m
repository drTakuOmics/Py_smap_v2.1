    ...
    case 2
        ...
        for i=1:nTemplates
            ...
            if( isreal(temp) )
                switch ccMode
                    case 'filt'
                        template=single(smap.cropOrPad(temp,[size(imref,1),size(imref,2)],median(temp(:))));
                        template=smap.nm(template);
                end;
                template_F=fftn(ifftshift(template))./(prod(size(template)));%-mean(template(:))));
    ...
    case 3
        for i=1:nTemplates
            temp=templateref(:,:,:,i);
                    ...
                    template=single(smap.cropOrPad(temp,fullX.*dims_mat,median(temp(:))));
                    template=smap.nm(template);
                    template=single(smap.cropOrPad(temp,fullX.*dims_mat,median(temp(:))));
